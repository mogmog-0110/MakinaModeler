@echo off
REM Writes the geometry half of Grasp3D's POV export for every .gsf, as the reference for the C++ port.
...
"%JDK%\javac.exe" -encoding UTF-8 -cp "%GRASP%\bin" -d "%OUT%" "%HERE%PovDump.java"
if errorlevel 1 (
...
for %%f in ("%GRASP%\*.gsf" "%VERIFY%\*.gsf") do (
    "%JDK%\java.exe" -cp "%CP%" PovDump "%%f" "%OUT%\%%~nf.pov"

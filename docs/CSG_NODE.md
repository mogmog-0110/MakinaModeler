...
この表は 256 ノード時代の計算。現在の `Scene` 別名は **4096 ノード（約 390 KB）** —
ネットの実シーン（scene.pov、氷のマクロ展開で実測 2285 ノード）を 1:1 で取り込むための
インポート容量で、手編集の規模ではない。エンジンが毎フレームのスナップショットを持つ
段になったら、`SceneStorage` がテンプレートである点を使って巻き戻し用の小さい実体を
別途選び、境界で変換する（Scene.hpp の別名コメント参照）。上の 28 KB という予算は
その小実体側の話として今も生きている。
